...
class Game:
	...
	def tick(self):
		...
		if not self.is_over:
			...
			for pid, agent in self._agents.items():
				...
				player_state = self._get_player_state(pid)
				action = self._get_agent_input(pid, agent, game_state, player_state)
				if action: 
	...
	def generate_map(self, seed=1):
		...
		start_location = [(0,0), (6, 6)]
		iloc = numpy.random.permutation(2)
		for i, player in enumerate(self.players.values()):
			player.pos = start_location[iloc[i]]
	...
	def _get_agent_input(self, pid, agent, game_state, player_state):
		player = self.players[pid] if pid in self.players else None
		if not player or not player.is_alive:
			return False

		logger.debug(f"[{player.name}] agent move....")
		chosen_move = agent.next_move(game_state, player_state)
	...
	def _get_player_state(self, pid):
		...
		info = [self.players[p] for p in self.players.keys() if p != pid]
		
		player_state = self._player_state(pid, player, info)
		
		return player_state

	def _player_state(self, id:PID, player:_Player, info):
		return PlayerState(id=id, ammo=player.ammo, hp=player.hp, location=player.pos, reward=player.reward, power=player.power, info=info) \
			if player else None
