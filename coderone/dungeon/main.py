...
from .agent_driver.agent import AgentProxy
...
TICK_STEP = 0.0 			# Number of seconds per 1 iteration of game loop
...
def __load_or_generate_config(config_file:Optional[str]) -> dict:
	...
	config_data.setdefault('run_n_step', 0)
...
def run_match(agents:List[str], players:List[str]=None, config_name:str=None, record_file:str=None, watch:bool=False, args:Any=None):
	...
	if args:
		...
		if args.run_n_step or 'run_n_step' not in config:			config['run_n_step'] = args.run_n_step
...
def main(raw_input=None):
	...
	# Added to run game for n steps
	parser.add_argument('--run_n_step', type=int, help=argparse.SUPPRESS)
