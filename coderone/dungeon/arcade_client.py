...
# ------Redifinition
PADDING = (int(WIDTH/2), int(HEIGHT/128))
		...
		for column in range(self.game.column_count):
			for row in range(self.game.row_count):
				if column == 2 and row == 2:
					sprite = arcade.Sprite(self.asset_man.roud_corner_tile, 4)
					x, y = grid_to_pos((column, row))
					sprite.set_position(x, y)
					self.grid_sprite_list.append(sprite)
					continue
				if column == 2 and row == 4:
					sprite = arcade.Sprite(self.asset_man.roud_corner90_tile, 4)
					x, y = grid_to_pos((column, row))
					sprite.set_position(x, y)
					self.grid_sprite_list.append(sprite)
					continue
				if column == 4 and row == 4:
					sprite = arcade.Sprite(self.asset_man.roud_corner180_tile, 4)
					x, y = grid_to_pos((column, row))
					sprite.set_position(x, y)
					self.grid_sprite_list.append(sprite)
					continue
				if column == 4 and row == 2:
					sprite = arcade.Sprite(self.asset_man.roud_corner270_tile, 4)
					x, y = grid_to_pos((column, row))
					sprite.set_position(x, y)
					self.grid_sprite_list.append(sprite)
					continue
				if column == int(self.game.column_count / 2) and row == int(self.game.row_count / 2):
					# Condition for the center for pavilion
					sprite = arcade.Sprite(self.asset_man.pavilion_tile, 4)
					x, y = grid_to_pos((column, row))
					sprite.set_position(x, y)
					self.grid_sprite_list.append(sprite)
					continue
				if column == 3 and (row == 2 or row == 4):
					sprite = arcade.Sprite(self.asset_man.roud_horizontal_tile, 4)
					x, y = grid_to_pos((column, row))
					sprite.set_position(x, y)
					self.grid_sprite_list.append(sprite)
					continue
				if row == 3 and (column == 2 or column == 4):
					sprite = arcade.Sprite(self.asset_man.roud_straight_tile, 4)
					x, y = grid_to_pos((column, row))
					sprite.set_position(x, y)
					self.grid_sprite_list.append(sprite)
					continue
				else:
					if column == int(self.game.column_count/2) or row == int(self.game.row_count/2):
						# Condition for the special Cross region x=3 or y=3
						if row == 3:
							# condition for horizontal line in the Cross
							sprite = arcade.Sprite(self.asset_man.roud_horizontal_tile, 4)
							x, y = grid_to_pos((column, row))
							sprite.set_position(x, y)
							self.grid_sprite_list.append(sprite)
						else:
							# condition for vertical line in the Cross
							sprite = arcade.Sprite(self.asset_man.roud_straight_tile, 4)
							x, y = grid_to_pos((column, row))
							sprite.set_position(x, y)
							self.grid_sprite_list.append(sprite)
					# The rest will just be grass
					else:
						sprite = arcade.Sprite(self.asset_man.grass_tile, 4)
						x,y = grid_to_pos((column, row))
						sprite.set_position(x, y)
						self.grid_sprite_list.append(sprite)
